...
2. GH Pages = **정적 미리보기만** 명시
3. 로컬 검증: backend `:5000`
4. env는 **키 이름만** 안내 (값 저장·출력 금지)
5. APP_URL 배포 URL (메일)
...
**A. 문서/설정** — DEPLOY · yaml · env.example 동기화  
**B. 배포 실패** — rootDir, static root, getApiBase, Mongo 로그  
**C. 메일 링크** — APP_URL, appUrl.js  
**D. Pages only** — workflow, getBasePath, GITHUB_STATIC  
