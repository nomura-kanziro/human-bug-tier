...
- 풀이 화면에 보이면 `#pool-viewport-arrows` (▲ 티어표, ▼ 풀 끝). PNG 캡처 밖
- 데스크톱 DnD, 모바일 **탭 선택 후 칸 탭**
- 본인 글 수정: 상세 공유하기·신고하기 사이 **수정하기** → `post_edit.html`(수정완료) → PUT → 상세
- 업로드/수정 모달: 제목·내용·썸네일. 상세는 제목 아래 내용
- 캐릭터 id는 이름 기반 안정 id
- 티어표 꾸미기: 액션바 **꾸미기** → 테두리 색·배경 이펙트·등급별 테마. `tierData.style.byGrade`로 저장, PNG/상세 복원
