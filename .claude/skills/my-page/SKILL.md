...
- `common.js` (`renderUserProfile` 이하 프로필 패널 함수들)
...
2. 일반 유저 + 어드민 둘 다 드롭다운. 어드민 메뉴 구성(관리하기/로그아웃)은 `admin` 스킬 소관 — 마이페이지 메뉴는 일반 유저 전용
...
- [ ] 비공개 글 노출 범위(본인만) 확인
- [ ] 비로그인 접근 시 리다이렉트
- [ ] 어드민 드롭다운(관리하기/로그아웃) 정상, 마이페이지 메뉴 없음
