---
name: hbu-react-rewrite
description: >
  정식 버전 React 이식. 바닐라 기능 100% 반영 완료, 배포 전환은 지시 대기.
---

# 에이전트 스킬 — React 정식 버전 (`root-cloudflare/`)
...
5. `requireAdmin`·SHA-256 재설정·토큰 규칙 유지 (프론트 가드는 UX 용, 최종 판단은 서버)
6. 검증: `npm run build`(root-cloudflare) → `npm start`(backend) → `:5000`

## Do not

지시 없이 React 실무 배포 전환, `src/styles/*.css` 직접 수정, Workers 이식, CF 배포 재개.
