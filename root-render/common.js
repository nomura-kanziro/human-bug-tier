...
// 그 안에 있는 <a href="/xxx">나 <img src="tier-media/tier-image/...">처럼 "루트 기준" 경로는
...
function fixRootLinksInElement(container) {
  ...
  container.querySelectorAll('img[src], link[href]').forEach(el => {
    ...
    if (val.startsWith('/')) {
      ...
      // e.g. "tier-media/tier-image/xx.png" or just "logo.webp" treat as root relative
...
function getProfileImageSrc() {
  ...
  return getBasePath() + 'tier-media/tier-image/logo.webp';
}
...
function bindProfileImageFallback(img) {
  if (!img) return;
  const fallback = getBasePath() + 'tier-media/tier-image/logo.webp';
...
function fixImagePaths(base) {
  ...
  if (logoImg) {
    logoImg.src = base + 'tier-media/tier-image/logo.webp';
