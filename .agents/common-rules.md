...
## ０. ⛔ 작업 대상 프론트 (2026-09-19 창시자 지시 — 최우선)

| 폴더 | 상태 | 규칙 |
|------|------|------|
| **`root-cloudflare/`** (React) | ✅ **유일한 작업 대상** | 모든 프론트 신규 기능·버그수정을 여기서만 한다 |
| **`root-render/`** (바닐라) | ⛔ **작업 금지 (베타 종료)** | 베타 버전에서 업데이트 **종료**. 수정·추가·리팩터 전부 금지 |
| `backend/` | ✅ 작업 가능 | 두 프론트 공용 API 서버 — 기존대로 유지·개선 |

1. **`root-render/` 아래 어떤 파일도 고치지 않는다.** 기능 추가·버그 수정·CSS 보정·리팩터 전부 해당.
2. 사용자가 기능을 요청하면 **묻지 말고 `root-cloudflare/`(React)에만** 구현한다.
3. 실수로 `root-render/`를 수정했으면 **되돌리고** 사람에게 알린다.
4. 바닐라를 정본으로 보던 기존 절차(`npm run sync:render` 등)는 **더 이상 돌리지 않는다** — 돌리면 React 쪽 수정이 바닐라 구본으로 덮어쓰여진다.
5. 예외는 **창시자의 명시적 지시**가 있을 때뿐.

> 배경: Render.com 배포는 베타 버전까지만 유지하고, 정식 버전은 React(`root-cloudflare/`)로 간다.
...
5. **기존 주석 삭제 금지.** 코드가 바뀌면 주석 **내용만** 고친다. 사람이 “주석 지워”라고 하기 전에는 블록·줄 주석을 없애지 않는다. 새 기능이면 설명 주석을 유지·추가한다.  
...
- ❌ **`root-render/` 수정** (베타 종료 — 작업은 `root-cloudflare/`에만)
- ❌ `.env` 커밋/출력  
- ❌ 관리 API 무인증  
- ❌ getBasePath/getApiBase 무시 URL  
- ❌ GH Pages = 풀 API  
- ❌ 요청 없는 전 파일 리포맷  
- ❌ **기존 주석을 리팩터·정리 명목으로 삭제** (사람은 “지워”할 때만, AI는 내용 수정만)  
